...
const Gem = function(ctx, x, y, color) {
    ...
    //     sprite.setXY(x, y);
    // };
    const generateGem = function(x, y, color) {
        setColor(color);
        sprite.setXY(x, y);
    };

    const newGem = function(area) {
        const colors = ["green", "red", "yellow", "purple"];
        gemColor = colors[Math.floor(Math.random() * 4)];

        /* Randomize the position */
        const {x, y} = area.randomPoint();

        Socket.getSocket().emit("generate new gem", x, y, gemColor);
    };
